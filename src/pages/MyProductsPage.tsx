...
import { ArrowRight, BarChart3, Package, ReceiptText } from "lucide-react";
...
function MyProductsContent({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    getUserProducts(userId).then(setProducts).catch((err) => setError(err instanceof Error ? err.message : "Nao foi possivel carregar seus produtos."));
  }, [userId]);
